...
function setup_sprite(loopOpt, bbox_offsetOpt, bbox_sizeOpt, img_src, max_frames, max_ticks, frame_size, src_offset) {
  ...
  let bbox_size$1 = Primitive_object.equal(bbox_size, [
    0,
    0
...
function update_animation(spr) {
  ...
  if (curr_ticks >= spr.params.max_ticks) {
    ...
    if (spr.params.loop) {
      spr.frame.contents = Primitive_int.mod_(spr.frame.contents + 1 | 0, spr.params.max_frames);
      return;
    } else {
      return;
    }
...
function update_vel$1(obj) {
  if (obj.grounded) {
    ...
    obj.vel.y = Primitive_float.min(obj.vel.y + 0.2 + Math.abs(obj.vel.y) * 0.01, 4.5);
    return;
  } else {
    return;
  }
...
function update_loop(canvas, param, map_dim) {
  ...
  let update_helper = (time, state, player, objs, parts) => {
    ...
    draw_bgd(state.bgd, Primitive_int.mod_(vpos_x_int, bgd_width));
