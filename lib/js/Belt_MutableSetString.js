...
let Belt_SortArrayString = require("./Belt_SortArrayString.js");
let Belt_internalSetString = require("./Belt_internalSetString.js");
...
function add(d, k) {
  let oldRoot = d.data;
  let v = Belt_internalSetString.addMutate(oldRoot, k);
  if (v !== oldRoot) {
    d.data = v;
    return;
  }
...
function split(d, key) {
  ...
  let i = Belt_SortArrayString.binarySearch(arr, key);
  let len = arr.length;
  if (i >= 0) {
    return [
      [
...
function intersect(dataa, datab) {
  ...
  let k = Belt_SortArrayString.intersect(tmp, 0, sizea, tmp, sizea, sizeb, tmp2, 0);
  return {
...
function diff(dataa, datab) {
  ...
  let k = Belt_SortArrayString.diff(tmp, 0, sizea, tmp, sizea, sizeb, tmp2, 0);
  return {
    data: Belt_internalAVLset.fromSortedArrayAux(tmp2, 0, k)
  };
}
...
function union(dataa, datab) {
  ...
  let k = Belt_SortArrayString.union(tmp, 0, sizea, tmp, sizea, sizeb, tmp2, 0);
  return {
    data: Belt_internalAVLset.fromSortedArrayAux(tmp2, 0, k)
  };
}
